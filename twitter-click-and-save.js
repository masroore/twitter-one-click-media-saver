// ==UserScript==
// @name        Twitter One Click Media Saver
// @version     0.0.1-2023.03.11
// @namespace   max.twitter
// @description Add buttons to download images and videos in .
// @match       https://twitter.com/*
// @homepageURL https://github.com/masroore/twitter-one-click-media-saver
// @supportURL  https://github.com/masroore/twitter-one-click-media-saver/issues
// @license     GPL-3.0
...
if (
  globalThis.GM_registerMenuCommand /* undefined in Firefox with VM */ ||
...
function loadSettings () {
  ...
  try {
    savedSettings =
      JSON.parse(localStorage.getItem('ujs-click-n-save-settings')) || {}
  } catch (e) {
    console.error('[ujs]', e)
...
function showSettings () {
      ...
      <div class="ujs-modal-settings" style="${modalSettingsStyle}">
          <fieldset>
              ...
              <label><input type="checkbox" ${
                s.hideTrends ? 'checked' : ''
              } name="hideTrends">Hide <b>Trends</b> (in the right column)*<br/></label>
              <label><input type="checkbox" ${
                s.hideSignUpSection ? 'checked' : ''
              } name="hideSignUpSection">Hide <b title='"New to Twitter?" (If yoy are not logged in)'>Sign Up</b> section (in the right column)*<br/></label>
              <label><input type="checkbox" ${
                s.hideSignUpBottomBarAndMessages ? 'checked' : ''
              } name="hideSignUpBottomBarAndMessages">Hide <b>Sign Up Bar</b> and <b>Messages</b> (in the bottom)<br/></label>
              <label hidden><input type="checkbox" ${
                s.doNotPlayVideosAutomatically ? 'checked' : ''
              } name="doNotPlayVideosAutomatically">Do <i>Not</i> Play Videos Automatically</b><br/></label>
              <label hidden><input type="checkbox" ${
                s.goFromMobileToMainSite ? 'checked' : ''
              } name="goFromMobileToMainSite">Redirect from Mobile version (beta)<br/></label>
              <label title="Makes the button more visible"><input type="checkbox" ${
                s.addBorder ? 'checked' : ''
              } name="addBorder">Add a white border to the download button<br/></label>
              <label title="Hides the modal login pop up. Useful if you have no account. \nWARNING: Currently it will close any popup, not only the login one.\nIt's reccommended to use only if you do not have an account to hide the annoiyng login popup."><input type="checkbox" ${
                s.hideLoginPopup ? 'checked' : ''
              } name="hideLoginPopup">Hide <strike>Login</strike> Popups (beta)<br/></label>
          </fieldset>
          <fieldset>
              <legend>Recommended</legend>
              <label><input type="checkbox" ${
                s.highlightVisitedLinks ? 'checked' : ''
              } name="highlightVisitedLinks">Highlight Visited Links<br/></label>
              <label title="In most cases absolute links are 3rd-party links"><input type="checkbox" ${
                s.highlightOnlySpecialVisitedLinks ? 'checked' : ''
              } name="highlightOnlySpecialVisitedLinks">Highlight Only Absolute Visited Links<br/></label>

              <label title="Note: since the recent update the most NSFW spoilers are impossible to expand without an account"><input type="checkbox" ${
                s.expandSpoilers ? 'checked' : ''
              } name="expandSpoilers">Expand Spoilers (if possible)*<br/></label>
          </fieldset>
          <fieldset>
              <legend>Highly Recommended</legend>
              <label><input type="checkbox" ${
                s.directLinks ? 'checked' : ''
              } name="directLinks">Direct Links</label><br/>
              <label><input type="checkbox" ${
                s.handleTitle ? 'checked' : ''
              } name="handleTitle">Enchance Title*<br/></label>
          </fieldset>
          <fieldset ${isFirefox ? '' : 'style="display: none"'}>
              <legend>Firefox only</legend>
              <label title='${downloadProgressFFTitle}'><input type="radio" ${
      s.downloadProgress ? 'checked' : ''
    } name="firefoxDownloadProgress" value="downloadProgress">Download Progress<br/></label>
              <label title='${strictTrackingProtectionFixFFTitle}'><input type="radio" ${
      s.strictTrackingProtectionFix ? 'checked' : ''
    } name="firefoxDownloadProgress" value="strictTrackingProtectionFix">Strict Tracking Protection Fix<br/></label>
          </fieldset>
          <fieldset>
              <legend>Main</legend>
              <label><input type="checkbox" ${
                s.imagesHandler ? 'checked' : ''
              } name="imagesHandler">Image Download Button<br/></label>
              <label><input type="checkbox" ${
                s.videoHandler ? 'checked' : ''
              } name="videoHandler">Video Download Button<br/></label>
              <label hidden><input type="checkbox" ${
                s.addRequiredCSS ? 'checked' : ''
              } name="addRequiredCSS">Add Required CSS*<br/></label><!-- * Only for the image download button in /photo/1 mode -->
          </fieldset>
          <fieldset>
              <legend title="Outdated due to Twitter's updates, or impossible to reimplement">Outdated</legend>
              <strike>

              <label title="It seems Twitter no more shows this section."><input type="checkbox" ${
                s.hideTopicsToFollow ? 'checked' : ''
              } name="hideTopicsToFollow">Hide <b>Topics To Follow</b> (in the right column)*<br/></label>
              <label title="Prevent the tweet backgroubd blinking on the button/image click. \nOutdated. \nTwitter have removed this disgusting behavior. This option is more no need."><input type="checkbox" ${
                s.preventBlinking ? 'checked' : ''
              } name="preventBlinking">Prevent blinking on click (outdated)<br/></label>

              <label hidden><input type="checkbox" ${
                s.hideTopicsToFollowInstantly ? 'checked' : ''
              } name="hideTopicsToFollowInstantly">Hide <b>Topics To Follow</b> Instantly*<br/></label>
              </strike>
          </fieldset>
          <hr>
  ...
  document
    .querySelector('body > .ujs-modal-wrapper .ujs-save-setting-button')
...
function ujs_getGlobalFetch ({ verbose, strictTrackingProtectionFix } = {}) {
  const useFirefoxStrictTrackingProtectionFix =
    strictTrackingProtectionFix === undefined
      ? true
      : strictTrackingProtectionFix // Let's use by default
  const useFirefoxFix =
    useFirefoxStrictTrackingProtectionFix &&
    typeof wrappedJSObject === 'object' &&
    typeof wrappedJSObject.fetch === 'function'
...
<div class="ujs-btn-common ujs-btn-error-text"></div>`.slice(1)
    ...
    static _preventBlinking (clickBtnElem) {
      const weakSet =
        Features.hasBlinkListenerWeakSet ||
        (Features.hasBlinkListenerWeakSet = new WeakSet())
      let wrapper
    ...
    static async _downloadBanner (url, btn) {
      ...
      const { blob, lastModifiedDate, extension, name } = await fetchResource(
        url
      )

      Features.verifyBlob(blob, url, btn)

      const filename = `${username} ${id} ${seconds}.${extension}`
    ...
    static async _imageClickHandler (event) {
      ...
      const filename = `${author}-${name}.${extension}`
      downloadBlob(blob, filename, url)
    ...
    static async _videoClickHandler (event) {
      ...
      try {
        ;({
          ...
          screenName: author,
          vidNumber
        } = await API.getVideoInfo(id, author, posterUrl))
    ...
    static async directLinks () {
      ...
      const anchors = xpathAll(
        `.//a[@dir="ltr" and child::span and not(@data-handled)]`
      )
      for (const anchor of anchors) {
    ...
    static hideTopicsToFollowInstantly () {
      if (!I18N.TOPICS_TO_FOLLOW) {
        // Unsupported lang, no TOPICS_TO_FOLLOW constant
        return
      }
      addCSS(`
    ...
    static hideTopicsToFollow () {
      ...
      const elem = xpath(
        `.//section[@role="region" and child::div[@aria-label="${I18N.TOPICS_TO_FOLLOW}"]]/../..`
      )
      if (!elem) {
        return
      }
...
function hoistAPI () {
  class API {
    ...
    static async apiRequest (url) {
      ...
      verbose &&
        console.log('[ujs][apiRequest]', JSON.stringify(json, null, ' '))
    ...
    static async getVideoInfo (tweetId, screenName, posterUrl) {
      ...
      const isVideoInQuotedPost =
        !tweetData.extended_entities ||
        tweetData.extended_entities.media.findIndex(
          e => e.media_url_https === posterUrl
        ) === -1
      ...
      let mediaIndex = tweetData.extended_entities.media.findIndex(
        e => e.media_url_https === posterUrl
      )

      if (vidNumber === -1 || mediaIndex === -1) {
        verbose &&
          console.log('[ujs][warning]: vidNumber === -1 || mediaIndex === -1')
        vidNumber = 0
        mediaIndex = 0
      }
      const videoVariants =
        tweetData.extended_entities.media[mediaIndex].video_info.variants
    ...
    static async getUserInfo (username) {
      ...
      const url = `https://twitter.com/i/api/graphql/${qHash}/UserByScreenName?variables=${encodeURIComponent(
        variables
      )}`
      const json = await API.apiRequest(url)
